...
/**
 * Builder class for the <code>LR1ParserTable</code>. Instances of this class can be re-used to
...
public class LR1ParserTableBuilder {
     ...
     * @param pair pair to map the action to
     * @param action action to add
     */
    public void addAction(TablePair pair, ParserAction action){
        actionTable.put(pair, action);
    }
    
    /**
     * Gets the action mapped to the given pair.
     * @param pair the pair the action is mapped to
     ...
     */
    public boolean containsPair(TablePair pair){
        return getAction(pair) != null;
    }
    
    /**
     * Builds the table. Calling this method will also reset the builder so it can be re-used to
     * build multiple tables.
     * @return
     */
    public LR1ParserTable build(){
        LR1ParserTable result = new LR1ParserTable(actionTable);
        reset();
        return result;
    }
